...
Mat contours_find(Mat src, Mat dst, std::vector<std::vector<Point>> *contours) {
  ...
  cv::waitKey(0);
  //在边缘添加矩形边
  cv::rectangle(src, Point(0, 0), Point(src.cols, src.rows),
                Scalar(255, 255, 0), 30, 4, 0);
  ...
  for (int i = 0; i < contours->size(); i++) {
    drawContours(dst, *contours, i, Scalar(255, 255, 255), 5, 10, hierarchy, 1,
                 Point(0, 0));
  }
...
void Bundle_sort(Point3d pa[], int count_Pointsize) {
  for (int count_area1 = 0; count_area1 < count_Pointsize - 1; count_area1++) {
    ...
    for (int count_area2 = 0; count_area2 < count_Pointsize - 1 - count_area1;
         count_area2++) {
...
int compute_center_of_gravity(Mat dst2,
                              std::vector<std::vector<Point>> contours,
                              int contours_size, Point3d pa[]) {
  int i = 0;
...
void nms_point(Mat White, Point3d pa[], Point3d pb[], int count_Pointsize) {
  ...
  while (index_size > 0) {
    ...
    for (int count_point = 0; count_point < index_size - 1; count_point++) {
      int distance = sqrt(pow((pb[index_result].x - pa[count_point].x), 2) +
                          pow((pb[index_result].y - pa[count_point].y), 2));
      cout << "distance : " << distance << endl;
      //如果小于阀值,说明距离很接近,则覆盖掉
      if (distance < threshold_set) {
        // cout << " distance is too short" << endl;

        for (int count_point2 = count_point; count_point2 < index_size - 1;
             count_point2++) {
  ...
  for (int nms_point = 0; nms_point < index_result; nms_point++) {
    circle(White, p[nms_point], 1, Scalar(0, 255, 255), 30,
           8);  //原图画出重心坐标
...
int main(int argc, char **argv) {
  ...
  //计算连通区域的质心坐标
  count_Pointsize =
      compute_center_of_gravity(dst, contours, contours.size(), pa);
  // Bundle sort
